...
async function moveMouse(x, y) {
    try {
        ...
        return `🖱️ Fare ${x},${y} koordinatına taşındı.`;
    } catch (error) {
...
async function clickMouse(button = 'left') {
    ...
    try {
        ...
        return `🖱️ ${buttonText} tıklama simüle edildi.`;
    } catch (error) {
...
async function scrollMouse(direction = 'down', amount = 3) {
    try {
        ...
        for (let i = 0; i < amount; i++) {
            await execPromise(`powershell -Command "$wsh = New-Object -ComObject WScript.Shell; $wsh.SendKeys('${key}')"`);
            await new Promise(resolve => setTimeout(resolve, 100));
        }
        const dirText = direction === 'up' ? 'Yukarı' : 'Aşağı';
        return `🖱️ ${dirText} scroll yapıldı (${amount}x).`;
    } catch (error) {
...
async function sendKeys(keys) {
    try {
        ...
        await execPromise(`powershell -Command "$wsh = New-Object -ComObject WScript.Shell; $wsh.SendKeys('${escapedKeys}')"`);
        return `⌨️ Tuşlar gönderildi: "${keys}"`;
    } catch (error) {
...
async function typeText(text) {
    try {
        ...
        await execPromise(`powershell -Command "$wsh = New-Object -ComObject WScript.Shell; $wsh.SendKeys('${escapedText}')"`);
        return `⌨️ Metin yazıldı: "${text.substring(0, 50)}${text.length > 50 ? '...' : ''}"`;
    } catch (error) {
...
async function pressKey(key) {
    try {
        ...
        await execPromise(`powershell -Command "$wsh = New-Object -ComObject WScript.Shell; $wsh.SendKeys('${mappedKey}')"`);
        return `⌨️ Tuş basıldı: ${key}`;
    } catch (error) {
...
async function pressKeyCombination(combination) {
    try {
        ...
        await execPromise(`powershell -Command "$wsh = New-Object -ComObject WScript.Shell; $wsh.SendKeys('${sendKeysString}')"`);
        return `⌨️ Kombinasyon basıldı: ${combination}`;
    } catch (error) {
