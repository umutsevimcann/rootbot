...
async function startUSBMonitoring(callback) {
    try {
        if (usbMonitor) {
            return '⚠️ USB izleme zaten aktif.';
        }
...
function stopUSBMonitoring() {
    if (!usbMonitor) {
        return '⚠️ USB izleme zaten pasif.';
    }
...
async function startBatteryMonitoring(callback, threshold = 20) {
    try {
        if (batteryMonitor) {
            return '⚠️ Pil izleme zaten aktif.';
        }
...
function stopBatteryMonitoring() {
    if (!batteryMonitor) {
        return '⚠️ Pil izleme zaten pasif.';
    }
...
async function startNetworkMonitoring(callback) {
    try {
        if (networkMonitor) {
            return '⚠️ İnternet izleme zaten aktif.';
        }
...
function stopNetworkMonitoring() {
    if (!networkMonitor) {
        return '⚠️ İnternet izleme zaten pasif.';
    }
...
async function startCPUMonitoring(callback, threshold = 90, duration = 5) {
    try {
        if (cpuMonitor) {
            return '⚠️ CPU izleme zaten aktif.';
        }
...
function stopCPUMonitoring() {
    if (!cpuMonitor) {
        return '⚠️ CPU izleme zaten pasif.';
    }
