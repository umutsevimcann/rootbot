...
async function turnOffScreen() {
    try {
        ...
        return '🖥️ Ekran kapatıldı. Hareket ettirdiğinizde açılacak.';
    } catch (error) {
        ...
        if (stdout.trim()) {
            try {
                ...
                if (procList.length > 0) {
                    ...
                    msg += '\n⚠️ Bu uygulamalar mikrofonunuzu kullanıyor olabilir.';
                    return msg;
                }
            } catch (e) {
                // JSON parse hatası
...
async function showDesktop() {
    try {
        ...
        return '🖥️ Tüm pencereler minimize edildi. Masaüstü gösteriliyor.';
    } catch (error) {
...
async function getWiFiPassword() {
    try {
        ...
        return `📶 *WiFi:* ${profileName}\n\n⚠️ Şifre bulunamadı (açık ağ olabilir).`;
    } catch (error) {
...
async function emptyRecycleBin() {
    try {
        ...
        return '🗑️ Geri dönüşüm kutusu boşaltıldı.';
    } catch (error) {
        // Hata olsa bile boşaltma işlemi başarılı olabilir
        return '🗑️ Geri dönüşüm kutusu boşaltma işlemi tamamlandı.';
    }
}
...
async function setBrightness(level) {
    try {
        ...
        try {
            ...
            return `⚠️ Parlaklık ayarlanamadı.
...
function getQuickCommandsHelp() {
    return `⚡ *Hızlı Komutlar*\n\n` +
           ...
           `🖥️ Masaüstü Göster - Tüm pencereleri minimize et\n` +
           `📂 Son Dosyalar - Son açılan 10 dosya\n` +
           `📶 WiFi Şifresi - Bağlı WiFi şifresi\n` +
           `🗑️ Geri Dönüşüm Boşalt - Recycle bin temizle\n` +
